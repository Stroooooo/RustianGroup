...
class App {
	...
	private vulnerabilityProtection(): void {
		...
		this.express.use(
			cors({
				credentials: true,
				origin: ['http://localhost:3000', 'https://localhost:3000', 'localhost:8080', 'https://localhost:8080', "localhost:3000"],
				// Allow the custom authentication header from the dashboard client
				allowedHeaders: ['authentication', 'content-type', 'authorization'],
				// Expose the authentication header so the client can read it on login
