...
export class AuthBl {
	 ...
	 * Create user
	*/
	public async createUser(user: User): Promise<LoginResults> {
		// const salt = await bcrypt.genSalt(Configuration.keysHandling.bcryptSaltRounds);
		// user.password = await bcrypt.hash(user.password, salt);

		await this.usersBl.createUser(user);
