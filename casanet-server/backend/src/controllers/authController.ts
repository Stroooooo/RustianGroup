...
export class AuthController extends Controller {
	...
	private activeSession(key: string) {
		...
		// Allow the client to send and read the custom authentication header
		this.setHeader('Access-Control-Allow-Headers', 'authentication, Authorization, Content-Type');
		this.setHeader('Access-Control-Expose-Headers', 'authentication');

		const maxAgeInSec = sessionExpiresMs / 1000;
		// For cross-origin cookie usage SameSite must be 'None' and cookie must be Secure.
		// Use 'None' only when HTTPS is enabled, otherwise keep 'Strict' for local dev.
		const sameSite = Configuration.http.useHttps ? 'None' : 'Strict';
		const secure = Configuration.http.useHttps ? 'Secure' : '';
		this.setHeader('Set-Cookie', `${SESSION_COOKIE_NAME}=${key}; Max-Age=${maxAgeInSec}; Path=/; HttpOnly; ${secure}; SameSite=${sameSite};`);
	}
	...
	*/
	@Post('signup')
	@Response<void>(201, 'user created')
	@Response<ErrorResponse>(501, 'Server error')
	@Response<ErrorResponse>(422, 'Invalid schema')
	public async signup(@Request() request: express.Request, @Body() user: User): Promise<void> {
		console.log('Signup called');
		console.log(user);

		// try {
			await AuthBlSingleton.createUser(user);
			this.setStatus(201);
			return;
		// } catch (error) {
		// 	this.setStatus(501);
