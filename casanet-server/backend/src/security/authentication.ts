...
export const expressAuthentication = async (
	...
	if (!request.cookies[SESSION_COOKIE_NAME]) {
		logger.warn(`[expressAuthentication] No session cookie found for ${securityName}. Headers: ${JSON.stringify(Object.keys(request.headers))}`);
		throw {
	...
	try {
		logger.debug(`[expressAuthentication] Looking up session token for ${securityName}...`);
		const session = await SessionsBlSingleton.getSession(request.cookies.session);
		logger.debug(`[expressAuthentication] Session found for ${session.email}`);
