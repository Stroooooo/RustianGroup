...
function SignupForm() {
	...
	async function signup() {
		try {
			const authResponse = await ApiFacade.AuthenticationApi.signup({
		...
		<div className="login-form-header">
			...
			>
				<div className="login-form-header-icon">
					<img width={'150em'} height={'150em'} alt="casanet-logo" src={casanetLogo} />
				</div>
		...
		<div className="login-form-submit">
			{loading
				...
				: <Button disabled={loading} id="login-submit" onClick={submit} style={{ width: '100%' }} variant="contained" color="primary" >
					{t('signup.sign.up').toUpperCase()}
				</Button>}
		</div>
		<Button style={{ textAlign: 'center', width: '100%', marginTop: '2%' }} color="primary" onClick={() => navigate(AppRoutes.login.path)}>
			Have an account? {` ${t('login.sign.in')}`}
		</Button>
		<Grid
